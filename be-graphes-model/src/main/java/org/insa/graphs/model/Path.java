            ...
            for (Node current : nodes) {
                if (first) {
                    ...
                    for (Arc successor : successors) {
                        if (successor.getDestination().equals(current) && (time == 0 || successor.getMinimumTravelTime() <= time)) {
                            selected = successor;
                            time = successor.getMinimumTravelTime();
                        }
                    }
                    if (selected == null) {
                        throw new IllegalArgumentException();
        ...
        if (nodes.size() > 1) {
            for (Node current : nodes) {
                if (first) {
                    ...
                    successors = previous.getSuccessors();
                    for (Arc successor : successors) {
                        if (successor.getDestination().equals(current) && (length == 0 || successor.getLength() <= length)) {
                            selected = successor;
                            length = successor.getLength();
                        }
                    }
                    if (selected == null) {
                        throw new IllegalArgumentException();
